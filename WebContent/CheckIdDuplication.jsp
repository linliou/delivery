            ...
            session.setAttribute("idDuplicationChecked", false);    
         	...
         	if(!usertype.equals("1")){
         		if(CID==null || CID.equals("")) {
    	   	 		out.println("<script language=javascript> alert('아이디가 입력되지 않았습니다'); window.history.go(-1); </script>");
    	       		return;
                }       
                else if(MyDB.isDuplicatedCId(CID)) {     
    	   	 		out.println("<script language=javascript> alert('" + CID + ": 중복된 아이디로 사용할 수 없습니다'); window.history.go(-1); </script>");
    	       		return;
                }
                else {
                    ...
                    session.setAttribute("idDuplicationChecked", true);    
    	   	 		...
    	   	 		out.println("<script language=javascript> alert('" + CID + ": 사용 가능한 아이디입니다'); window.history.go(-1); </script>");
                }
