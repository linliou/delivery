...
<body> 
    <%
     	Boolean b = (Boolean) session.getAttribute("SIDDuplicationChecked");
                ...
                if (b == null || b == false) {
            	 		out.println("<script language=javascript> alert('아이디 중복 검사를 하지 않았습니다.'); window.history.go(-1); </script>");
                 		return;
                 } 
                else
                  ...
                  if (MyDB.insertAdmin(SID,SNAME,SADNAME,PW, CP ,ADD) && MyDB.insertStore(SNAME, CLASSTIFICATION, OPENTIME, CLOSETIME) ) {    
           	 		out.println("<script language=javascript> alert(" + SNAME +"점주님 아이디" + SID + "(으)로 회원 가입햇습니다.'); </script>");
