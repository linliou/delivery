...
<div align=right>
      <%=session.getAttribute("id")%>님이 로그인 했습니다.<br> <input type=button
         ...
         <button onclick="location.href='CustomerStart.jsp'">홈으로</button><br>
 ...
 <form name=form1 method=post action = CustomerReviewPrint.jsp >
    <table align=center valign=top border="1" cellpadding="8" cellspacing="0" bordercolor="#999999">
		...
		<tr bgcolor=#CCCCCC>		
			<th>주문번호</th><th>가게</th><th>메뉴</th><th>주문수량</th><th>요구사항</th><th>총 가격</th>
		</tr>
		<tr>
   		 	<td><%=ONUM %> </td><td><%=rs.getString("SNAME") %></td><td><%=rs.getString("MNAME") %></td><td><%=rs.getString("OCNT") %></td><td><%=rs.getString("REQUEST") %></td><td><%=rs.getString("TP") %></td>
   		</tr>
   </table>
 
<TABLE border=1 width=320 align=center valign=top bgcolor=white cellspacing=1 cellpadding=1 bordercolor=black> 
    ...
    <TR align=center>
	   		...
	   		<TR align=center ><TD colspan="2"><H4 align=center> <input type="submit" value=" 확 인"> </TD>
	</form>
</body>
</html>